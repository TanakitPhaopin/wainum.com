...
export default function Preview() {
    ...
    const fetchTeacher = async () => {
            const teacherId = user?.id; // Replace with actual teacher ID
            ...
            if (data) {
                ...
                || !data.bio
                || !data.hourly_rate
                || !data.contacts.some(contact => contact.type === 'phone' && contact.value)
                || !data.levels
                || !data.about_lesson
                || !data.about_location
                || !data.experience
                || !data.qualification
                || !data.swim_teacher_locations.some(location => location.province_code)) {
                    toast.error("ข้อมูลไม่ครบถ้วน กรุณาแก้ไขข้อมูลในหน้าโปรไฟล์");
    ...
    return (
        <div className='min-h-screen w-full'>
            ...
            <div>
                <div ref={section1Ref} className="relative container mx-auto p-4 rounded-xl shadow-xl bg-[#ffffff]">
                    ...
                    <div className="md:flex md:grid-cols-2 md:gap-2 md:justify-evenly md:items-center">
                        ...
                        <div className="flex flex-col md:flex-row items-center md:justify-start md:gap-2">
                            <h1 className='text-center text-2xl font-bold text-wrap'><span>{is_subscribed && (<VerifiedIcon color='primary'/>)}</span> {teacher.display_name}</h1>
                ...
                {/* Gallery */}
                {is_subscribed && teacher.swim_teacher_gallery && teacher.swim_teacher_gallery.length > 0 && (
            ...
            {/* Introduction video */}
            {is_subscribed && teacher.video_link && (
